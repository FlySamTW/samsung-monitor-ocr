...
- 第一輪完整可立即定案；遠景、FollowMe、缺型號／價格或核心證據有疑慮時，第二、第三輪都只看同一張原圖，不得收到前輪答案。
- 三輪後每張照片都取得如實結果並立即逐張上傳。遠景、單機只有型號、只有價格、兩者皆無都可完成；1–2 台完整螢幕卻被模型判遠景的矛盾，在三輪皆為同圖健康判讀時保守定案為單機且不猜欄位。內容不確定不再等待慢模型或未指定的人工作業。
- 完整螢幕只從第一張原始全圖計算一次，外框四邊四角都在原圖內才算；碰到或穿出原圖邊界的左右鄰機不計。後續裁切圖是同圖定位證據，不得重複計數。中央完整、左右裁切的構圖只有在全圖沒有其他完整螢幕時才是 1 台；一整排、展示牆、多層貨架或寬廣走道不得套用這個近拍例外。兩輪無型號／無價格的弱單機票不能壓掉一輪有效的 3+ 遠景結構證據。
- 兩輪同主體 FollowMe 強實體證據可確認 FollowMe 家族；M5／M7／Pro 無兩輪共識時輸出 `FollowMe_型號未細分`，不可退回遠景或猜版本。
- 只有前輪答案外洩、跨照片身分複製、提示詞汙染、request/image 綁定失敗或 Drive 回讀失敗等技術完整性證據可停止批次。同類單張視覺矛盾在不同照片重複，不等於記憶汙染。
- 單一照片的一次 request ID 缺失／不符只作廢該輪並占用同張三次額度，不立即拖停所有照片；相同錯誤出現在另一張不同照片時，才視為系統性串線並熔斷整批。壞回覆永遠沒有內容投票權。
- 正式介面目前維持在既有分頁 `http://127.0.0.1:5002/`；50/50 照片／AI 判讀區、右側累積縮圖、全案與本批進度不得任意更動。
...
# 2026-07-17 Revision .35 Live Recovery

- Every photo has at most three stateless, request-bound calls. A human-audited full-image SHA authority may settle only on call 3 after all three calls prove the same image, independence, no prior-answer exposure and no prompt contamination; it never creates a fourth call.
