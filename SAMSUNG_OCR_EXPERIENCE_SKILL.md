...
The contract version and guard implementation identity are separate authorities. Every result and trace produced by the complete three-layer rules must carry `evidence_guard_revision=20260715.7`. A legacy `v19.45 verified` trace without that revision is unverified under the current rules and must be emitted by the backfill builder; success CSV, rerun completion, Label Studio metadata, and Drive manifests must all preserve and check the revision. Migration must never invent the revision for evidence that was not actually evaluated by it. Revision `.7` includes `.5` stateless pass 2/3 and runtime health fuse plus `.6` discontinued/unlisted SKU photo consensus. A retailer short SKU may expand only to one unique known model by appending 1–3 trailing characters, never by changing the size or middle of the token, and still requires a second independent matching model/price/label pass. Auto-verification of a fully unlisted SKU requires three independent identical model/price passes and at least two `unique_main=true`, `label_ownership=matched` passes; otherwise the value remains reviewable and upload stays closed. No prior assistant response, classification, model, price, reason, price-conflict hint, or live mistake-book answer may enter a review prompt.
