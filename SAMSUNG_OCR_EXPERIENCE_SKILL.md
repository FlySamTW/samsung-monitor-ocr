...
The contract version and guard implementation identity are separate authorities. Every result and trace produced by the complete three-layer rules must carry `evidence_guard_revision=20260715.5`. A legacy `v19.45 verified` trace without that revision is unverified under the current rules and must be emitted by the backfill builder; success CSV, rerun completion, Label Studio metadata, and Drive manifests must all preserve and check the revision. Migration must never invent the revision for evidence that was not actually evaluated by it. Revision `.5` includes the `.3` raw-Samsung-SKU/final-other-brand conflict guard and `.4` structured FollowMe evidence fix. It also makes pass 2 and pass 3 stateless: no prior assistant response, classification, model, price, reason, price-conflict hint, or live mistake-book answer may enter a review prompt. Transport retries rebuild the pristine image request. Dialogue-style correction language, raw JSON in the narration panel, missing narration, or runtime content conflicts activate the health fuse, stop processing, and keep upload closed.
