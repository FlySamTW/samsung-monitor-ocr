...
The contract version and guard implementation identity are separate authorities. Every result and trace produced by the complete three-layer rules must carry `evidence_guard_revision=20260715.6`. A legacy `v19.45 verified` trace without that revision is unverified under the current rules and must be emitted by the backfill builder; success CSV, rerun completion, Label Studio metadata, and Drive manifests must all preserve and check the revision. Migration must never invent the revision for evidence that was not actually evaluated by it. Revision `.6` includes the `.3` raw-Samsung-SKU/final-other-brand conflict guard, `.4` structured FollowMe evidence fix, and `.5` stateless pass 2/3 plus runtime health fuse. It also preserves a discontinued/unlisted SKU as a photo-evidence candidate only when the current structured answer and readable same-subject physical label agree. Auto-verification requires three independent passes with identical model and price and at least two passes with `unique_main=true` and `label_ownership=matched`; otherwise the value remains reviewable and upload stays closed. No prior assistant response, classification, model, price, reason, price-conflict hint, or live mistake-book answer may enter a review prompt. Transport retries rebuild the pristine image request. Dialogue-style correction language, raw JSON in the narration panel, missing narration, or runtime content conflicts activate the health fuse, stop processing, and keep upload closed.
