...
The contract version and guard implementation identity are separate authorities. Every result and trace produced by the complete three-layer rules must carry `evidence_guard_revision=20260716.14`. A legacy `v19.45 verified` trace without that revision is unverified under the current rules and must be emitted by the backfill builder. Revision `.14` includes `.5` stateless pass 2/3 and runtime health fuse, `.6` discontinued/unlisted SKU photo consensus, `.8` short-SKU completion, `.9` complete distant evidence with semantic cross-pass comparison, `.10` narration support without integer duplication, `.11` explicit FollowMe negation handling, `.12` common-schema evidence fields, `.13` sub-three distant rejection, and `.14` pre-rescue structured authority. An explicitly present `model` or `price` field, including null, may never be refilled from narration; prose rescue is legacy-only when that field is absent. A dominant centered complete monitor with a readable spatially aligned label/price remains a single-unit candidate even when one neighboring monitor is partial or visible. A retailer short SKU may expand only to one unique known model by appending 1–3 trailing characters, never by changing the size or middle of the token; the pipeline-owned completion marker must survive post-processing and force a second independent matching model/price/label pass. Auto-verification of a fully unlisted SKU still requires three independent identical model/price passes and at least two `unique_main=true`, `label_ownership=matched` passes. For distant views, every pass must explicitly provide count >=3, `unique_main=false`, no matched main label, and no strong same-subject FollowMe evidence. Exact counts of 3 or more and the non-matched ownership states are semantically equivalent only for cross-pass comparison; a null/sub-three count, non-false unique main, matched label, or strong FollowMe evidence stays fail-closed. Narration must independently describe a multi-screen/row layout and no unique main; it does not need to duplicate the exact structured integer, but an explicit zero/one/two-complete statement remains a hard conflict. The word FollowMe remains a positive risk cue unless it is locally and explicitly negated by wording such as no/not seen/none; structured physical evidence always has priority over narration. Missing markers, prior-answer exposure, ambiguous completion, or failed consensus keeps upload closed.
