...
- A bare `S32FM...` or `S43FM...` SKU is a Smart Monitor panel identity, not proof of a FollowMe mobile bundle. Establish FollowMe first from explicit on-unit branding or sufficient same-subject fixture evidence; only then may the SKU map M5/M7/Pro equivalence. After three calls, any disagreement among FollowMe variant/price pairs clears both fields and finalizes the truthful family result as `FollowMe（型號未細分）／無價格`.
