...
The contract version and guard implementation identity are separate authorities. Every result and trace produced by the complete three-layer rules must carry `evidence_guard_revision=20260715.8`. A legacy `v19.45 verified` trace without that revision is unverified under the current rules and must be emitted by the backfill builder. Revision `.8` includes `.5` stateless pass 2/3 and runtime health fuse plus `.6` discontinued/unlisted SKU photo consensus. A retailer short SKU may expand only to one unique known model by appending 1–3 trailing characters, never by changing the size or middle of the token; the pipeline-owned completion marker must survive post-processing and force a second independent matching model/price/label pass. Auto-verification of a fully unlisted SKU still requires three independent identical model/price passes and at least two `unique_main=true`, `label_ownership=matched` passes. Missing markers, prior-answer exposure, ambiguous completion, or failed consensus keeps upload closed.
