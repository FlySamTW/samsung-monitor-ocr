...
The contract version and guard implementation identity are separate authorities. Every result and trace produced by the complete three-layer rules must carry `evidence_guard_revision=20260715.3`. A legacy `v19.45 verified` trace without that revision is unverified under the current rules and must be emitted by the backfill builder; success CSV, rerun completion, Label Studio metadata, and Drive manifests must all preserve and check the revision. Migration must never invent the revision for evidence that was not actually evaluated by it. Revision `.3` also compares the final normalized model with `raw_objects`: a final `它牌(...)` cannot pass when the model's structured object contained a Samsung SKU, and an explicitly negated screen-content brand must not overwrite the physical Samsung product.
