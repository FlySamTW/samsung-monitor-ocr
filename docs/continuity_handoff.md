...
5. 不得清空歷史、不得將前輪答案驅入後輪、不得用舊守門結果冒充 `.21` 完成。安全重跑前必須先快照舊證據，再從同一原圖無記憶重做。
...
9. 第一、第二、第三輪的三層即時守門，必須遵循 [three_layer_accuracy_gate.md](three_layer_accuracy_gate.md)。內容不一致由 `.21` 定案器保守收旂；只有請求綁定、跨圖汙染、系統輸出或 Drive 回讀等技術錯誤可不上傳該張。
