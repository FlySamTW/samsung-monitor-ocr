...
Three independent passes are the end of the same-model automatic retry lane. If their core evidence still disagrees, mark the photo unresolved, isolate it, and continue the batch; never start an unbounded fourth pass or count it as verified. Operator UI must say `三輪衝突／已隔離` and `待慢模型或人工最終裁決`, not the ambiguous `判讀未完成／待複核`. After a fuse smoke finishes, the backend still points at the smoke directory. Before relaunching `rerun_staged_candidates.py --resume-existing-then-continue`, safely switch the idle backend to the exact uniquely matched formal staging leaf with `/api/set_work_dir`; otherwise the runner must and will refuse because the current directory is outside its staging root.
