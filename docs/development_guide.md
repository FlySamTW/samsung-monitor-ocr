...
- Rebuild with `tools\build_drive_correction_reconciliation.py --output-dir ...` after the fresh 2026 manifest exists. Dry-run is the default; `--execute` atomically replaces the UTF-8 ledger only when every stale row maps to exactly one current `copied.csv` source and one manifest row. Missing historical IDs are filled later by the read-only `reconcile_drive_corrections.py --execute --phase discover-old`; a same-name/same-hash remote becomes `unchanged_remote_verified` and is never sent to the trash phase.
