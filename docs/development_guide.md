...
- `structured_authority_blocked_fields` is persisted with the parsed result when a legacy heuristic attempted such a rewrite. Monitor this field as content drift evidence. Any non-empty value means the gate prevented a post-processing override and the photo must remain reviewable unless a later independent pass produces internally consistent structured evidence.
