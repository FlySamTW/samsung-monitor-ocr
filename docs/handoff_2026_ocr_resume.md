...
- Script entrypoint: `python tools\rclone_drive_upload.py --execute --repeat --limit 100 --transfers 4 --checkers 8 --rclone-timeout-seconds 1200`.
- If a single rclone batch stalls, stop only the rclone/uploader process and resume with the same small batch plus timeout; do not stop OCR.
