...
5. 改名工具會依檔名 `年月` 強制清除歷史年度的比價符號；若接手者改過 planner，要先跑 `tools\test_photo_rename_planner.py`。
...
驗收摘要會寫到 `_ocr_audit\audit_summary.json`，內含驗收時間、審計檔路徑與主要數量；若驗收失敗，先看 `_ocr_audit\audit_report.csv`，不要直接宣稱完成。
