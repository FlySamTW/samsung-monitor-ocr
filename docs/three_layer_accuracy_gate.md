...
| 2026 FollowMe 單機 | 第 1 輪 | 唯一主角、自己的型號／價格與價牌歸屬一致，且 FollowMe 身分綁定同一實機的直接品牌證據或至少兩項獨立強實體線索 | 宣傳畫面或附近立牌不能單獨通過；缺欄位或有疑點才進第 2／3 輪 |
...
- 價牌歸屬不明、唯一主角不明或品質不合格。
- 型號、價格、結構證據與敘述互相矛盾。
- 官方參考價只在 OCR 後以固定程式產生 `↑`／`↓`／`✓`；價差本身不得觸發 VLM 重讀。只有價牌模糊、荒謬數值或型號／價格歸屬衝突才升級。
- FollowMe 候選：同一主體的型號、價格、價牌歸屬與白色垂直支架、圓形底座、託盤或同等實體證據都完整且互不矛盾，可在第一輪定案；宣傳立牌單獨不算，缺漏或矛盾才升級。
...
- 證據完整、型號價格俱全且互不矛盾的 2026 FollowMe 單機可在第一輪通過；缺漏或疑似誤綁才排第二輪。
- 2026 遠景必須完成三輪獨立複核。
- 結構說單機、敘述說遠景，或相反的衝突不能定案。
- `view_type/category` 衝突與價牌歸屬敘述衝突不能定案。
- FollowMe 正式 SKU 不得繞過同主體實體證據；但同主體證據、型號與價格第一輪均完整時不強制浪費第二輪。
- 畫面播放它牌 Demo 不得覆蓋清楚的 Samsung 實體 SKU。
- 大幅官方價差只標示 `↑`／`↓`，不得單獨觸發額外模型呼叫；照片上的實際售價照實保留。
...
| FollowMe 型號、價格與同主體實體證據完整可第一輪通過 | `test_complete_current_year_followme_can_finish_on_first_pass` |
| FollowMe 正式 SKU 仍不能繞過同主體實體證據 | `test_smart_monitor_sku_needs_fixture_evidence_to_become_followme` |
...
| 大幅官方價差只做箭頭註記、不觸發重讀 | `test_large_official_price_difference_is_annotation_not_retry` |
...
- 模型回覆必須帶回本次完整 128-bit 隨機 `request_id`。缺少或不一致的回覆整輪作廢、不得進入內容證據，但仍占用同張最多三次的呼叫額度；同張只可用剩餘額度從原圖無記憶重試。單一來源首次發生時只隔離該回覆；若不同來源再次發生同類綁定錯誤，才視為系統性故障並熔斷整批。最後健康門本身還必須看到 `request_id_verified=true` 與 64 碼全圖 SHA-256，不可只信上游宣稱已驗證。
...
## `.41` 單張綁定故障隔離與三輪內容結案（2026-07-17）

- `request_id_missing/mismatch` 的該輪沒有投票權，不能顯示成正常判讀，也不能上傳；但第一次只證明該張該輪壞掉，不足以讓整批永久停機。
- 同張剩餘輪次仍受總數三次硬上限。第三次仍是綁定錯誤時，該張留下「技術失敗」結果，批次進下一張；不同照片再出現同類錯誤才觸發全域 fuse，避免把真正系統性串線放過。
- 三輪皆同圖、request-bound、獨立、無前輪答案、無提示污染且證據契約健康時，只要至少兩輪一致確認唯一一般單機，即使型號與價格都看不到，也應結案為 `單機／無型號／無價格` 並逐張上傳。不得誤標為技術錯誤，也不得等待第四輪、慢模型或未指定人工。
- `M-台北市-信義區-集雅社-統一時代二-150.jpg` 與 `M-台中市-西屯區-集雅社-台中大遠百-389.jpg` 是永久回歸案例：兩張都必須以 `two_pass_identity_free_single_consensus` 結案，不增加模型呼叫。
- 本次完整 critical regressions 必須為退出碼 0，且正式批次要能在單張綁定錯誤後跨到下一來源、Dashboard 保持在線，才可解除 fuse。
