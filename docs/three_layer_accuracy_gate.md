...
`v19.45` 只代表證據欄位契約，不能單獨證明當時已執行目前完整的三層守門。每筆新結果與 trace 必須同時帶有 `evidence_guard_revision=20260715.8`。`.8` 保留 `.3`–`.6` 的衝突、FollowMe、跨輪隔離與未收錄型號照片共識，並完成 `.7` 價牌短碼補全的全管線接線：短碼只可補成唯一、只差 1–3 個尾碼字元的正式型號，內部「已補全」證據標記必須穿過後處理、trace 與即時守門，強制第二輪相同型號、價格與價牌歸屬才可通過。非唯一、字串中段不同、尺寸不同或標記遺失一律不得驗證。
