...
`v19.45` 只代表證據欄位契約，不能單獨證明當時已執行目前完整的三層守門。每筆新結果與 trace 必須同時帶有 `evidence_guard_revision=20260715.7`。`.7` 保留 `.3` 的原始 Samsung SKU／最終它牌衝突守門、`.4` 的 FollowMe 結構化物理證據修正、`.5` 的跨輪記憶隔離／執行期健康熔斷與 `.6` 的未收錄型號照片共識；另允許把價牌短碼補成唯一、只差 1–3 個尾碼字元的正式型號，但必須再有第二輪相同型號、價格與價牌歸屬才可通過。非唯一、字串中段不同或尺寸不同一律不得補全。未達條件仍保留讀值供複核，不得冒充「沒有規格牌」或授權上傳。
