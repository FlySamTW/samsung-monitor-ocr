...
`v19.45` 只代表證據欄位契約，不能單獨證明當時已執行目前完整的三層守門。每筆新結果與 trace 必須同時帶有 `evidence_guard_revision=20260715.6`。`.6` 保留 `.3` 的原始 Samsung SKU／最終它牌衝突守門、`.4` 的 FollowMe 結構化物理證據修正與 `.5` 的跨輪記憶隔離／執行期健康熔斷，並修正停產或官網未收錄型號的過度清除：只有結構答案與同張主角實體價牌明確一致時才保留為候選；必須三輪獨立判讀的型號與價格完全一致、至少兩輪 `unique_main=true` 且 `label_ownership=matched`，才可通過。未達共識仍保留照片讀值供複核，不得冒充「沒有規格牌」或授權上傳。
