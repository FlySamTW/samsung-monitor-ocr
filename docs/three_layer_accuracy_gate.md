...
- 第三輪後由 `.23` 定案器決定遠景或單機；無共識的型號／價格保留為空，不得猜測，但照片仍是已完成結果。兩輪同主體 FollowMe 強實體證據可確認 FollowMe 家族；版本無共識時標示 `FollowMe 型號未細分`，不可退回遠景。三輪皆為同圖健康判讀、但遠景輪只數到 1–2 台完整螢幕而無法形成安全視角共識時，保守定案為單機且不猜型號／價格，不得冒充技術錯誤。每張最多三次模型呼叫，技術錯誤也不得產生第四次呼叫或第四輪卡片。
