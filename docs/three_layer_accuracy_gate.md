...
`v19.45` 只代表證據欄位契約，不能單獨證明當時已執行目前完整的三層守門。每筆新結果與 trace 必須同時帶有 `evidence_guard_revision=20260716.14`。`.14` 保留 `.3`–`.13` 的衝突、FollowMe、跨輪隔離、未收錄型號照片共識、價牌短碼補全、遠景語意比較、共用證據 Schema 與「0／1／2 台完整入鏡永遠不是遠景」；並把結構答案權威提前到任何獨白補值之前。只要本輪結構物件明示 `model` 或 `price`，即使值為 null，敘述都不得用鄰近價牌文字補入；只有舊格式完全缺少該結構欄位時才保留保守救援。中央完整主螢幕與其空間對齊的可讀價牌構成單機候選，即使旁邊另有局部或次要螢幕也不得抹掉唯一主角。遠景模型輸出必須明示實際完整入鏡台數至少為 3、`unique_main=false`、無主角自有價牌且無同主體 FollowMe 強證據；跨輪比較只把各輪都至少 3 台的精確數量視為同一個 `3+` 證據。可讀敘述必須獨立說明整排／多台螢幕且無唯一主角；敘述若明說只有 0、1、2 台完整入鏡仍屬硬衝突。「無 FollowMe／未看到 FollowMe」等緊鄰明確否定不再被誤當正向線索，未否定的 FollowMe 文字與結構化強實體證據仍封鎖。任一輪少於 3、null、`unique_main` 不是 false、價牌 matched 或有同主體 FollowMe 強證據仍一律不得驗證。舊 `.13` trace 不得冒充 `.14`。
...
## 已隔離矛盾與整批停機（2026-07-16）

三層守門最後必須把每張照片分成「可驗證」或「待複核」，不能把矛盾冒充成功；但批次監控也不能把已安全隔離的待複核結果誤當成整批失控。若結構化欄位與敘述矛盾，且結果沒有明確 `auto_review_required`／`evidence_unresolved`／人工複核狀態，代表矛盾可能漏出守門，必須停止整批。若結果已明確待複核，該張仍禁止自動接受與上傳，並記入 `contained_review_conflicts`，但後續照片可繼續接受獨立守門。

`20260715.11` 七張隔離驗證的原圖預期為 4 張真遠景、3 張反例；實際結果正好是 4 張 verified、3 張 unresolved，21 輪皆未暴露前輪答案且無 prompt contamination。這組結果同時驗證「不能錯放」與「不能因正確攔下而讓整批白跑」。
