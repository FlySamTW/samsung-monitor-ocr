...
`v19.45` 只代表證據欄位契約，不能單獨證明當時已執行目前完整的三層守門。每筆新結果與 trace 必須同時帶有 `evidence_guard_revision=20260716.12`。`.12` 保留 `.3`–`.11` 的衝突、FollowMe、跨輪隔離、未收錄型號照片共識、價牌短碼補全、遠景語意比較、敘述不必複誦整數與 FollowMe 明確否定規則；並把四個證據欄位正式列進所有分支共用 JSON Schema，要求每輪都輸出，不得只寫在 narration。遠景模型輸出必須明示實際完整入鏡台數至少為 3、`unique_main=false`、無主角自有價牌且無同主體 FollowMe 強證據；跨輪比較只把各輪都至少 3 台的精確數量視為同一個 `3+` 證據。可讀敘述必須獨立說明整排／多台螢幕且無唯一主角；敘述若明說只有 0、1、2 台完整入鏡仍屬硬衝突。「無 FollowMe／未看到 FollowMe」等緊鄰明確否定不再被誤當正向線索，未否定的 FollowMe 文字與結構化強實體證據仍封鎖。任一輪少於 3、null、`unique_main` 不是 false、價牌 matched 或有同主體 FollowMe 強證據仍一律不得驗證。舊 `.11` trace 不得冒充 `.12`。
