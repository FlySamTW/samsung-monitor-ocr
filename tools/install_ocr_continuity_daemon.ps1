...
$cmd='powershell.exe -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File "{0}" -RepoRoot "{1}" -SourceRoot "{2}" -OutputDir "{3}" -BackendUrl "{4}"' -f $daemon,$RepoRoot,$SourceRoot,$OutputDir,$BackendUrl
function Get-Daemon {
...
function Start-Daemon {
  Start-Process -FilePath $ps -ArgumentList @('-NoProfile','-WindowStyle','Hidden','-ExecutionPolicy','Bypass','-File',$daemon,'-RepoRoot',$RepoRoot,'-SourceRoot',$SourceRoot,'-OutputDir',$OutputDir,'-BackendUrl',$BackendUrl) -WorkingDirectory $RepoRoot -WindowStyle Hidden | Out-Null
}
if($Action -eq 'ensure') {
