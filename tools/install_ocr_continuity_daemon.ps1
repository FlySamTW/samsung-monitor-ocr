param([ValidateSet("install","uninstall","status","ensure")][string]$Action="install",[string]$RepoRoot,[string]$SourceRoot,[string]$OutputDir,[string]$BackendUrl="http://127.0.0.1:5002")
...
$cmd='powershell.exe -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File "{0}" -RepoRoot "{1}" -SourceRoot "{2}" -OutputDir "{3}" -BackendUrl "{4}" -IntervalSeconds 60' -f $daemon,$RepoRoot,$SourceRoot,$OutputDir,$BackendUrl
function Get-Daemon {
...
function Start-Daemon {
  Start-Process -FilePath $ps -ArgumentList @('-NoProfile','-WindowStyle','Hidden','-ExecutionPolicy','Bypass','-File',$daemon,'-RepoRoot',$RepoRoot,'-SourceRoot',$SourceRoot,'-OutputDir',$OutputDir,'-BackendUrl',$BackendUrl,'-IntervalSeconds','60') -WorkingDirectory $RepoRoot -WindowStyle Hidden | Out-Null
}
if($Action -eq 'ensure') {
...
$tr='wscript.exe //B //Nologo "{0}" "{1}" "{2}" "{3}" "{4}"' -f $hiddenEnsure,$RepoRoot,$SourceRoot,$OutputDir,$BackendUrl
